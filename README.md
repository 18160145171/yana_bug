...
- 按正式测试报告结构输出可直接分享的 `.docx` Word 报告，版式包含版本信息、图表化分布、测试结论、优化建议和 Bug 明细。
...
## 报告版式
- 报告结构参考正式功能测试报告：一、版本信息；二、Bug优先级分布；三、Bug功能模块分布；四、Bug执行人分布；五、测试结论与建议；六、Bug详细列表。
- 统计章节包含优先级环形图、功能模块柱状图和对应明细表；没有可用中文字体时，图表会保留图形和数值，中文分类仍在 Word 表格中完整展示。
- AI 开启后，模型结论、风险和行动建议会写入第五章；接口失败时不会生成固定模板报告。

## 备注
- CSV 建议使用 `UTF-8 with BOM` 编码，避免中文乱码。
- 报告使用 Word `.docx` 格式输出，建议使用 Word 或 WPS 打开。
